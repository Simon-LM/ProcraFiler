...
The money figure is honest about what it knows. It always carries **the date of the rates it used**, because no provider publishes machine-readable prices — the table ships with the app, and you can override it with your own `pricing.json` in the config directory (see [docs/ai-pricing-source.md](docs/ai-pricing-source.md)). A model missing from the table is reported as *"AT LEAST"*, never priced at zero.

The published table keys each rate by **the name the seller's own page uses** — `mistral small 4`, `ocr 4.1 / ocr` — not by the API model id you configure. ProcraFiler records which **family** each of its models is sold under and reads the generation from the table on every run, so `ocr 4` becoming `ocr 4.1`, or a future Small 5, is followed with nothing to update. It records only the models ProcraFiler configures: that table is shared with other projects, so the models it lists beyond those are not ours to guess at. If a model has no price, `procrafiler doctor` says which one, and the forecast refuses to quote a total rather than invent one. When a rate has left the seller's price list, the run says so and names the day it was last published — the figure is still the best estimate available, and it is no longer a current rate. And the token weight of an **image** cannot be predicted from any published formula, so the first estimate is a rough default and says so — after that, the app uses **what your own runs actually consumed**, measured from each provider reply, so the forecast calibrates itself on your documents at your camera's resolution.
...
The updater fetches the tags and moves to the **latest release tag** (`vX.Y.Z`) — never an in-progress branch HEAD — then reinstalls in the venv. All of that happens inside the installation's own source copy, so **your clone is never fetched into, checked out, or moved**, and updating still works after you have deleted it. An installation made by an older installer — which recorded only your clone — is **repaired on the spot**: its source is copied out of that clone once, and never read from it again.

**Going back a version is allowed, and no longer silent.** A release that breaks something for you should not trap you on it, so `update.sh --ref v0.11.0` moves you back. What that used to do without a word was open a catalog a newer release had written. The state directory now records which release wrote it, and an older one **refuses** rather than write over it — naming both versions and the state it declined to touch. `PROCRAFILER_ALLOW_OLDER_VERSION=1` runs anyway if you have a reason to. The other direction is never blocked: a newer release opening an older state is an ordinary update, and the catalog migrates itself. It prints `old → new`. **Your library, catalog, settings and env file are never touched**, and the catalog migrates in place — so an update never forces you to reorganize anything. `procrafiler --version` always matches the installed release.
