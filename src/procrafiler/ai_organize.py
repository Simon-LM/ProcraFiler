...
def _build_organize_prompt(
    ...
    return (
        ...
        "- Only a one-off AFFAIR/event folder is dated. A SERIES (recurring kind) is filed in its "
        "ENTITY folder (issuer/organism — EDF, Enercoop, BNP-Paribas — or the kind when there is no "
        "issuer — Releves-eau, CV), NEVER dated and WITHOUT a year — the system appends the dated "
        "year subfolder itself (e.g. you give \".../Utilities/EDF\", it files into "
        "\".../Utilities/EDF/2026\"). DIFFERENT entities are DIFFERENT series → DIFFERENT folders (an "
        "EDF bill and an Enercoop bill never share a folder).\n"
        ...
        "bill, tax notice…) goes into its ENTITY folder (no year — the system dates it), even from a "
        "single instance (e.g. \".../Utilities/EDF\", \".../Utilities/Releves-eau\").\n"
