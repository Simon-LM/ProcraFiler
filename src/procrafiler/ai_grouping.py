...
def _build_grouping_prompt(
    ...
    return (
        ...
        "- HIGH BAR: only deepen/group when the files share the SAME SPECIFIC subject, series or "
        "affair — NOT merely the same broad parent category. Two unrelated items that happen to sit "
        "under the same branch (e.g. a sound-mixer manual and a co-development workshop, both under "
        "Hobbies) are DIFFERENT subjects: leave them as separate siblings, do NOT nest one under the "
        "other. When in doubt, confirm the original path and leave \"group_with\" empty.\n"
        "- A SERIES subfolder is named after its ENTITY (issuer/organism — EDF, Enercoop, "
        "BNP-Paribas — or the kind when there is no issuer — Releves-eau) and is NEVER dated and "
        "carries NO year — the system appends the dated year subfolder itself (you propose e.g. "
        "\"branch/Releves-eau\", it files into \"branch/Releves-eau/2024\"). Only a one-off AFFAIR "
        "folder is dated, its DATE at the START (e.g. \".../Housing/2025-08_Degats-eaux\"), NEVER at "
        "the end.\n"
        ...
        "- NAME A SHARED FOLDER BY WHAT ITS FILES HAVE IN COMMON, never after ONE file's specific "
        "brand, model, or title. A folder named after a single entity is correct ONLY when EVERY "
        "file in it is from that SAME entity (e.g. one issuer's bills). When you group items that are "
        "the same KIND but different brands/models (e.g. an SWR amplifier manual and a Soundcraft "
        "mixer manual), name the shared folder for the generic KIND (e.g. \"Materiel-audio\"), NEVER "
        "after one of them (\"SWR\").\n"
