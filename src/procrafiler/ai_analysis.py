...
def _build_analysis_prompt(text: str, base_categories: list[str], existing_paths: list[str]) -> str:
    ...
    return (
        ...
        "Fields:\n"
        "- \"name\": a short, specific French title that identifies THIS exact document among "
        "similar ones (no extension). Lead with its most distinctive entity — the person, "
        "organization, or subject it concerns (e.g. a CV → the person's name; a bill → the issuer) "
        "— then the key detail. Do NOT name it by its file type or format, and avoid empty words "
        "like \"document\", \"fichier\", \"texte\".\n"
