...
- **Deleting a document then re-adding it is no longer mistaken for a duplicate — and its data no longer lingers (deletion tombstone, default mode).** When you deleted a filed document and ran `rescan`, its catalog row was kept as a full `DELETED` record (name, path, fiche, hash). Re-dropping the same file in the inbox was then matched by hash and trashed as a "duplicate" — you could not re-file something you had deleted. Two changes fix this. (1) **De-duplication now ignores tombstones**: only a *live* (non-deleted) document counts as a duplicate, so a re-deposit is filed normally — and you're told, in the run, that you had previously deleted this content (logged as `redeposit_of_deleted`). (2) **A deleted row is reduced to a tombstone** — only its `id`, content `hash` and deletion date are kept; the name, path and the whole fiche are dropped, so nothing of a deleted document's content remains in the catalog (privacy), while the hash still lets a later re-deposit be recognised. On every deletion the file's **mirror copy and hidden text sidecar are quarantined to `Mirror_Trash`** (TTL-purged), and the action log keeps a trace (`library_file_deleted`, `library_deleted_mirror_quarantined`, `library_deleted_sidecar_quarantined`). New `has_live_sha256` / `deleted_at_for_sha256` / `tombstone_document` on the catalog; new `_trash_deleted_artifacts`. A future `purge` mode (drop the hash + id too, for the rare case where even the fingerprint must not survive) is the next slice. `tests/test_catalog.py` +1, `tests/test_pipeline.py` +1, `tests/test_rescan.py` +1, `tests/test_sidecars.py` −1.
