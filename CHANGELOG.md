...
- **A file you resolve via `review` now also gets its hidden text sidecar** — it was written *after* a parked file returned early, so review-resolved documents were invisible to deep search; it's now written before parking and moved out with the document on resolution. (#70)
